...
export default function OmokGameRoom() {
  ...
  useEffect(() => {
    ...
    const { enter_code, session_key, game_config } = state || {};
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (response.coordinate?.gameState) {
          const gameState = response.coordinate.gameState;
  ...
  const handleStartGame = () => {
    const hasOpponent = opponentPlayer && opponentPlayer.nickname !== '대기 중...' && opponentPlayer.nickname !== '';
    if (hasOpponent) {
      setGameStarted(true);
    }
  };
