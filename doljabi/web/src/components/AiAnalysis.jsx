...
import { historyToSgf, buildSgf, downloadSgf, defaultSgfFilename } from '../utils/sgf';

const COLS_LABEL = 'ABCDEFGHJKLMNOPQRST';

/**
 * gameData (App.jsx에서 전달):
 *   { history, gameType, winner, endedByDraw, blackName, whiteName,
 *     initialBlackSec, initialWhiteSec, endedAt }
 *
 * history / gameType props는 backwards-compat용으로 따로 받음.
 */
export const AiAnalysis = ({ onNavigate, gameType = 'go', currentUser, history = [], gameData = null }) => {
  ...
  const handleDownload = () => {
    ...
    const meta = {
      ...
      timeLimit: gameData?.initialBlackSec, // 최초 제한시간(초)
      date: gameData?.endedAt || new Date(),
      result: gameData?.endedByDraw
        ? '0'
        : gameData?.winner
          ? { winner: gameData.winner }
          : undefined,
    };
    const fullSgf = buildSgf(history, meta);
    downloadSgf(fullSgf, defaultSgfFilename({
      blackName: gameData?.blackName,
      whiteName: gameData?.whiteName,
      date: gameData?.endedAt || new Date(),
    }));
