...
type Board = Stone[][]; // ← added alias to avoid generic parsing issue
...
export default function BadukSinglePlay() {
  ...
  const [activeTab, setActiveTab] = useState<'game' | 'settings'>('settings');
  ...
  // Updated makeMove with AI thinking flag (modified)
  const makeMove = (row: number, col: number, stone: Stone) => {
    if (!stone) return;
    if (gameState !== 'playing') return;
    if (board[row][col]) return; // 이미 돌이 있는 위치는 착수 불가
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (
      ...
      makeMove(row, col, playerColor); // 착수 실행
    }
  };
      ...
      {gameState === 'setup' ? (
          ...
          <div className="rounded-2xl p-8 w-full max-w-md border"
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {([9, 13, 19] as BoardSize[]).map((size) => (
                    <button
                      ...
                      style={{
                        backgroundColor: boardSize === size ? 'rgba(31, 111, 235, 0.2)' : '#141822',
                        borderColor: boardSize === size ? '#8ab4f8' : '#2a2a33',
                        color: '#e8eaf0',
              ...
              <div className="rounded-lg p-4 border"
                ...
                <h4 className="font-semibold mb-3" style={{ color: '#e8eaf0' }}>게임 규칙</h4>
                <div className="space-y-2 text-sm" style={{ color: '#9aa1ad' }}>
                  <p>• {boardSize}×{boardSize} 바둑판에서 진행</p>
                  <p>• 흑돌이 먼저 시작</p>
                  <p>• 상대방 돌을 둘러싸면 잡을 수 있음</p>
                  <p>• 양쪽 모두 수넘김하면 게임 종료</p>
                  <p>• 백돌에게 6.5점 덤 제공</p>
                </div>
              </div>
        ...
        <div className="flex h-[calc(100vh-88px)]">
          ...
          >
            {/* Tab Navigation */}
            <div
              className="flex mb-6 rounded-lg p-1 border"
              style={{
                backgroundColor: 'rgba(22,22,28,0.6)',
                borderColor: '#2a2a33',
              }}
            >
              <button
                ...
                className={`flex-1 py-2 px-4 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                  activeTab === 'game' ? 'text-white' : ''
                }`}
                style={{
                  backgroundColor: activeTab === 'game' ? '#1f6feb' : 'transparent',
                  color: activeTab === 'game' ? '#ffffff' : '#9aa1ad',
                }}
              >
                게임
              </button>
              <button
                ...
                style={{
                  ...
                  color: activeTab === 'settings' ? '#ffffff' : '#9aa1ad',
                }}
              >
                설정
              </button>
            ...
            {activeTab === 'settings' && (
              <div className="space-y-6">
                ...
                >
                  <h4 className="font-semibold mb-3" style={{ color: '#e8eaf0' }}>
                    보드 크기
                  </h4>
                  <div className="space-y-2">
                    {([9, 13, 19] as BoardSize[]).map((size) => (
                      <button
                        ...
                        className={`w-full py-2 px-3 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                          boardSize === size ? 'text-white' : ''
                        }`}
                        style={{
                          backgroundColor: boardSize === size ? '#1f6feb' : 'transparent',
                          color: boardSize === size ? '#ffffff' : '#9aa1ad',
                          border: boardSize === size ? 'none' : '1px solid #2a2a33',
                        }}
                      >
                        {size}×{size}
                      </button>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <button
                      onClick={() => setPlayerColor('black')}
                      className={`w-full py-2 px-3 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                        playerColor === 'black' ? 'text-white' : ''
                      }`}
                      style={{
                        backgroundColor: playerColor === 'black' ? '#1f6feb' : 'transparent',
                        color: playerColor === 'black' ? '#ffffff' : '#9aa1ad',
                        border: playerColor === 'black' ? 'none' : '1px solid #2a2a33',
                      }}
                    >
                      ⚫ 흑돌 (선공)
                    </button>
                    <button
                      onClick={() => setPlayerColor('white')}
                      className={`w-full py-2 px-3 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                        playerColor === 'white' ? 'text-white' : ''
                      }`}
                      style={{
                        backgroundColor: playerColor === 'white' ? '#1f6feb' : 'transparent',
                        color: playerColor === 'white' ? '#ffffff' : '#9aa1ad',
                        border: playerColor === 'white' ? 'none' : '1px solid #2a2a33',
                      }}
                    >
                      ⚪ 백돌 (후공)
                    </button>
                  ...
                  <div className="space-y-2">
                    {(['easy', 'normal', 'hard'] as Difficulty[]).map((level) => (
                      <button
                        ...
                        onClick={() => setDifficulty(level)}
                        className={`w-full py-2 px-3 rounded-lg transition-all cursor-pointer whitespace-nowrap ${
                          difficulty === level ? 'text-white' : ''
                        }`}
                        style={{
                          backgroundColor: difficulty === level ? '#1f6feb' : 'transparent',
                          color: difficulty === level ? '#ffffff' : '#9aa1ad',
                          border: difficulty === level ? 'none' : '1px solid #2a2a33',
                        }}
                      >
                        {level === 'easy' ? '쉬움' : level === 'normal' ? '보통' : '어려움'}
                      </button>
            ...
            {activeTab === 'game' && (
              <div className="space-y-6">
                ...
                >
                  <h4 className="font-semibold mb-2" style={{ color: '#e8eaf0' }}>
                    타이머
                  </h4>
                  <div className="flex items-center justify-center">
                    <div
                      className={`text-3xl font-mono ${timeLeft <= 10 ? 'text-red-400' : ''}`}
                ...
                <div className="space-y-3">
                  <button
                    ...
                    style={{
                      backgroundColor:
                        currentPlayer === playerColor && !isAiThinking ? '#141822' : '#0f1419',
                      borderColor:
                        currentPlayer === playerColor && !isAiThinking ? '#2a2a33' : '#1a1a1a',
                      color:
                        currentPlayer === playerColor && !isAiThinking ? '#e8eaf0' : '#9aa1ad',
                    }}
                    onMouseEnter={(e) => {
