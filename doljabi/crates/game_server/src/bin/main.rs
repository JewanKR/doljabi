...
use game_server::{
    ...
    soyul::{
        game_history_api::game_history_router, gamehistory_db::create_game_history_table_if_not_exists,
        session::SessionStore, soyul_login::login_router,
    },
...
async fn main() {
    ...
    // game_history 테이블이 없으면 미리 생성 (게임 종료 훅이 처음 실행될 때 race 방지)
    if let Ok(conn) = rusqlite::Connection::open("mydb.db") {
        if let Err(e) = create_game_history_table_if_not_exists(&conn) {
            eprintln!("⚠️ game_history 테이블 초기화 실패: {}", e);
        }
    }
    ...
    let router_list = OpenApiRouter::new()
        ...
        .merge(game_history_router().with_state(session_manager.clone()))
