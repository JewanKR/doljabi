    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.running === false && gameRunningRef.current) {
          ...
          // 승자 결정 (setWinner와 동일 로직)
          let resolvedWinner; // 'black' | 'white' | 'draw'
          let endedByDraw = false;
          if (board.drawOffer !== undefined) {
            endedByDraw = true;
            resolvedWinner = 'draw';
          } else if (board.theWinner !== undefined) {
            resolvedWinner = 'white'; // 백(1) 승리: proto가 인코딩함
          } else {
            resolvedWinner = 'black'; // 흑(0) 승리: proto 기본값이라 undefined로 수신됨
          }

          // setWinner는 UI 표시용 (숫자 코드 유지)
          if (endedByDraw) {
            // winner null 유지
          } else {
            setWinner(resolvedWinner === 'white' ? 1 : 0);
          }

          // 상위(App)로 SGF 직렬화에 필요한 모든 정보 전달
          const info = usersInfoRef.current;
          onSaveHistory && onSaveHistory({
            history: [...historyRef.current],
            gameType: actualGameTypeRef.current,
            winner: resolvedWinner,
            endedByDraw,
            blackName: info?.black?.userName || null,
            whiteName: info?.white?.userName || null,
            initialBlackSec,
            initialWhiteSec,
