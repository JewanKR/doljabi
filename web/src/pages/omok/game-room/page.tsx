...
export default function OmokGameRoom() {
    ...
    const [isInByoyomi, setIsInByoyomi] = useState({
        black: false,
        white: false,
    });
    ...
    const colorEnumToString = (
        color: Color,
    ): "black" | "white" | "free" | null => {
        switch (color) {
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            try {
                ...
                console.log(
                    "  - gameStart:",
                    response.gameStart ? "있음" : "없음",
                );
                console.log(
                    "  - gameState:",
                    response.gameState ? "있음" : "없음",
                );
                console.log(
                    "  - usersInfo:",
                    response.usersInfo ? "있음" : "없음",
                );
                ...
                console.log(
                    ...
                    response.turn,
                    "→",
                    colorEnumToString(response.turn),
                );
        ...
        */
                    ...
                    if (response.usersInfo.black) {
                        setPlayers((prev) => ({
                            ...
                            black: {
                                ...prev.black,
                                nickname:
                                    response.usersInfo!.black!.userName ||
                                    "흑돌",
                                rating:
                                    response.usersInfo!.black!.rating || 1500,
                            },
                        }));
                    }
                    ...
                    if (response.usersInfo.white) {
                        setPlayers((prev) => ({
                            ...
                            white: {
                                ...prev.white,
                                nickname:
                                    response.usersInfo!.white!.userName ||
                                    "백돌",
                                rating:
                                    response.usersInfo!.white!.rating || 1500,
                            },
                        }));
                    }
                ...
                if (response.coordinate) {
                    console.log(
                        "🎯 착수 응답 수신, success:",
                        response.coordinate.success,
                    );
                    if (!response.coordinate.success) {
                        // 착수 실패 처리 (흑돌일 때는 금수, 백돌일 때는 일반 오류)
                        if (myColor === "black") {
                            alert(
                                "⛔ 금수입니다! 이 위치에는 돌을 놓을 수 없습니다.",
                            );
                            console.warn("⛔ 금수 (흑돌)");
                        } else {
                            alert(
                                "❌ 착수할 수 없습니다. (이미 돌이 있거나 잘못된 위치입니다)",
                            );
                            console.warn("❌ 착수 실패");
                        }
                ...
                if (response.drawOffer) {
                    const opponentName =
                        response.drawOffer.userName || "상대방";
                ...
                if (response.theWinner !== undefined) {
                    ...
                    if (winner === "black" || winner === "white") {
                        ...
                        setTimeout(() => {
                            alert(
                                `🏆 ${winnerName}(${winner === "black" ? "흑돌" : "백돌"})이 승리했습니다!`,
                            );
                            navigate("/");
                        }, 500);
                    } else if (winner === "free") {
                ...
                if (
                    ...
                    error.message.includes("MAX_SAFE_INTEGER")
                ) {
    ...
    const handlePlaceStone = () => {
        ...
        const chaksuRequest: ClientToServer = {
            sessionKey: sessionKey,
            coordinate: {
                coordinate: coordinate,
            },
        };
        const encoded = ClientToServer.encode(chaksuRequest).finish();
    ...
    const handlePass = () => {
        ...
        const passRequest: ClientToServer = {
            sessionKey: sessionKey,
            passTurn: {},
        };
        const encoded = ClientToServer.encode(passRequest).finish();
        wsRef.current.send(encoded);
        console.log("⏭️ 패스 전송");
    };
    ...
    const handleResign = () => {
        ...
        // 서버로 기권 요청 전송
        const resignRequest: ClientToServer = {
            sessionKey: sessionKey,
            resign: {},
        };
        const encoded = ClientToServer.encode(resignRequest).finish();
        wsRef.current.send(encoded);
        console.log("🏳️ 기권 전송");
    };
    ...
    const handleDrawRequest = () => {
        ...
        // 서버로 무승부 신청 전송
        const drawRequest: ClientToServer = {
            sessionKey: sessionKey,
            drawOffer: {},
        };
        const encoded = ClientToServer.encode(drawRequest).finish();
        wsRef.current.send(encoded);
    ...
    const handleStartGame = () => {
        ...
        if (!canStartGame) {
            alert(
                "두 명의 플레이어가 모두 입장해야 게임을 시작할 수 있습니다.",
            );
            return;
        }
        ...
        const config = loadRoomConfig();
        const sessionKey =
            config?.session_key || SessionManager.getSessionKey();
        ...
        const gameStartRequest: ClientToServer = {
            sessionKey: sessionKey,
            gamestart: {},
        };
        const encoded = ClientToServer.encode(gameStartRequest).finish();
        wsRef.current.send(encoded);
  ...
  ============================================================================================ */
    ...
    const opponentColor =
        myColor === "black" ? "white" : myColor === "white" ? "black" : "white";
    const opponentPlayer = players[opponentColor];

    const isMyColor =
        myColor === "black" ? "black" : myColor === "white" ? "white" : "black";

    const myTimePercentage = myColor
        ? getTimePercentage(myPlayer.mainTime, initialTime[myColor])
        : 0;
    const opponentTimePercentage = getTimePercentage(
        opponentPlayer.mainTime,
        initialTime[opponentColor],
    );
            ...
            >
                <div className="flex items-center space-x-3">
                    <div
                        ...
                        style={{
                            background:
                                "linear-gradient(135deg, #1f6feb 0%, #1b4fd8 100%)",
                ...
                <div className="flex-1 max-w-4xl">
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                {Array.from({ length: boardSize - 1 }).map(
                                    (_, rowIndex) =>
                                        Array.from({
                                            length: boardSize - 1,
                                        }).map((_, colIndex) => (
                                ...
                                style={{
                                    ...
                                    boxShadow:
                                        selectedPosition && isMyTurn
                                            ...
                                            : "none",
                                    opacity:
                                        selectedPosition && isMyTurn ? 1 : 0.5,
                                    cursor:
                                        selectedPosition && isMyTurn
                                            ? "pointer"
                                            : "not-allowed",
                    ...
                    <div className="flex items-start space-x-3">
                        ...
                        >
                            ...
                            >
                                ...
                                />
                                <path
                                    d="M8 10C8 12 7 13 10 13C13 13 12 12 12 10"
                                    stroke="white"
                                    strokeWidth="1"
                                    strokeLinecap="round"
                                />
