...
export default function BadukCreateRoom() {
  ...
  const [roomCode] = useState(`BD${Math.random().toString(36).substr(2, 6).toUpperCase()}`);
  ...
  const BOARD_SIZE = 19;
  const CELL_SIZE = 43; // 바둑 빠른 대국과 동일한 크기 유지
  const BOARD_PADDING = 51; // 바둑 빠른 대국과 동일한 크기 유지
  ...
  return (
    <div className="min-h-screen text-white" style={{ backgroundColor: '#0b0c10' }}>
      ...
      <header className="flex items-center justify-between px-8 py-6 border-b" style={{ borderColor: '#2a2a33' }}>
        ...
        <h2 className="text-xl font-semibold" style={{ color: '#e8eaf0' }}>바둑 방 만들기</h2>
