...
/// Baduk(바둑)의 보드 한 변 길이. coord = row * BOARD_SIZE + col 로 인코딩.
...
pub struct BadukRoom {
    ...
    // --- SGF/기보 저장용 필드 ---
    /// DB의 game_history.game_id 로 기록될 12자 랜덤 ID.
    game_id: String,
    /// 게임 시작 Unix time(초). game_start() 시점에 세팅.
    started_at: i64,
    /// 한 판 동안의 모든 수순(착수+패스). DB에 JSON으로 저장.
    move_history: Vec<MoveRecord>,
    /// 동일 게임에 대해 record_finished_game이 두 번 호출되지 않게 가드.
    history_saved: bool,
}
impl BadukRoom {
    ...
    /// 게임 종료 시점에 game_history 테이블로 한 줄 INSERT.
    /// 중복 호출 방지를 위해 history_saved 플래그로 가드.
    fn save_to_history_db(&mut self, winner: Color) {
        ...
        // 게임 시작 시점의 레이팅을 따로 캐싱하지 않으므로, 종료 시점 기준 레이팅을 그대로 사용.
        // (정확한 "변동 전" 레이팅이 필요하다면 game_start에서 캐싱 필요)
        ...
        if let Err(e) = record_finished_game_with_moves(
            ...
            eprintln!("⚠️ 게임기록 INSERT 실패: {}", e);
        } else {
            #[cfg(debug_assertions)]
            println!(
                "✅ 게임기록 저장: game_id={}, 수순={}수",
                self.game_id,
    ...
    pub fn record_winner(&mut self, color: Color) {
        ...
        // game_history 테이블에 한 판 + 수순 저장 (멱등 — history_saved 가드)
        ...
        if let Some(GameDataForClient::Baduk(message)) = message.game_data {
            match message.payload {
                ...
                Some(PayloadForClient::PassTurn(_pass_request)) => {
                    ...
                    if self.pass_turn {
                        // 두 번째 패스 — 게임 종료. 기보에 패스 기록 후 승자 결정.
                        self.push_pass(turn);
