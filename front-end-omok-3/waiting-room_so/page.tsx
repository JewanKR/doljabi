...
export default function OmokWaitingRoom() {
  ...
  // URL 쿼리에서 enterCode 읽기 (?enterCode=123456)
  const searchParams = new URLSearchParams(location.search);
  const enterCodeParam = searchParams.get('enterCode') ?? undefined;

  const state = (location.state as WaitingRoomState | null) ?? null;

  // 방 코드: ① URL 쿼리 → ② state.roomCode → ③ 랜덤 코드
  ...
  // isHost: 명시적으로 false가 아니면 기본적으로 방장으로 취급
  ...
  const handleStartGame = () => {
    navigate('/omok/game-room', {
      state: { gameSettings, players, roomCode },
    });
  };
  ...
  const allPlayersReady =
    players.every((p) => p.ready) && players[1].nickname !== '대기 중...';
                ...
                >
                  <svg width="32" height="32" viewBox="0 0 32 32">
                    <circle
                      ...
                      cy="16"
                      r="14"
                      fill={player.color === 'black' ? '#000' : '#fff'}
                    />
                    {player.color === 'black' && (
                      <circle
                        cx="11"
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  style={{
                    backgroundColor: canEditSettings
                      ? '#141822'
                      : '#2a2a33',
                ...
                <input
                  ...
                  onChange={(e) =>
                    canEditSettings &&
                    handleTimeChange('mainTime', e.target.value)
                  }
                  disabled={!canEditSettings}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    canEditSettings &&
                    handleTimeChange('byoyomiTime', e.target.value)
                  }
                  disabled={!canEditSettings}
                  className="w-28 px-3 py-2 rounded text-center font-semibold"
                  style={{
